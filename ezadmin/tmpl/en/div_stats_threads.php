...
<div class="page_title">Statistics over discussions and comments</div>


<h4>Global statistics</h4>
<div class="container">
    <h5>Since the beginning  <?php echo "[$minCreationDate]"//echo $minDateFr->format('l j F Y');   ?></h5>
    <p><b><?php echo $threadsCount; ?></b> discussions have been created</p>
    <p><b><?php echo $commentsCount; ?></b> comments have been published</p>
</div>
<br/>
<h4>Repartition of discussions and comments by album</h4>
<p class="help-block">
    <span class="glyphicon glyphicon-warning-sign">The album has to contains at least one discussion to be displayed.</span> 
</p>
<center>
...
<!--            <input type="text" id="dpMonths" class="form-control" data-date-format="mm/yyyy" 
    ...
    <label id="month-search-error" class="label label-important" style="display: none; font-size: 1em;">The date you entered is later than the current date.</label>
    <br/>
</div>

<!-- S E A R C H   F O R   x L A S T   D A Y S -->
<br />
<h4>Statistics for the N last days</h4>
...
<label id="nDays-search-error" class="label label-important">Numeric value expected. Found NaN value.</label>
<br/>
        ...
        <form action="index.php?action=get_csv_assets" method="post" id="csv_assets_form" name="csv_assets_form" onsubmit="return false">
            <a id="submit-csvAssets-search" class="btn btn-info btn-search" 
               onclick="document.csv_assets_form.submit();
                       return false;"> <span class="icon-file icon-white"></span>Number of discussions by asset</a>
        </form>               
    </div>
    <br/>
</div>
