...
<table class="table table-striped table-bordered table-hover table-responsive table-condensed classrooms table-left">
    <tr>
        <th></th>
        <?php echo $colOrder->insertThSort("room_ID", "Room number"); ?>
        <?php echo $colOrder->insertThSort("name", "Room name"); ?>
        <?php echo $colOrder->insertThSort("IP", "IP address (master)"); ?>
        <?php echo $colOrder->insertThSort("IP_remote", "IP address (slave)"); ?>
        <th>Enabled</th>
        <th></th>
        <th></th>
    </tr>
        ...
        <tr>
            <td style="text-align: center;">
                ...
                if((!array_key_exists('online', $currClass) || !$currClass['online']) && 
                        $currClass['enabled']) {
                    echo '<span title="No answer to ping requests"><span class="glyphicon glyphicon-warning-sign"></span></span>';
                }
        ...
        if($this.hasClass('btn-success')) {
            $.ajax("index.php?action=enable_classroom", {
                ...
                success: function(jqXHR, textStatus) {
                    ...
                    if(data.error) {
                        if(data.error == '1') alert("Impossible to change the state of this room.");
                        return;
                    }
    ...
    $("table.classrooms .delete_button").click(function() {
        ...
        $.ajax("index.php?action=remove_classroom", {
             ...
             success: function(jqXHR, textStatus) {
                ...
                if(data.error) {
                    if(data.error == '1') alert("An error has occured during the classroom deletion.");
                    return;
                }
