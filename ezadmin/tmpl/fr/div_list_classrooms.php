...
<table class="table table-striped table-bordered table-hover table-responsive table-condensed classrooms table-left">
    <tr>
        <th></th>
        <?php echo $colOrder->insertThSort("room_ID", "Numéro de local"); ?>
        <?php echo $colOrder->insertThSort("name", "Nom du local"); ?>
        <?php echo $colOrder->insertThSort("IP", "Adresse IP (maitre)"); ?>
        <?php echo $colOrder->insertThSort("IP_remote", "Adresse IP (esclave)"); ?>
        <th>Activé</th>
        <th></th>
        <th></th>
    </tr>
        ...
        <tr>
            <td style="text-align: center;">
                ...
                if((!array_key_exists('online', $currClass) || !$currClass['online']) && 
                        $currClass['enabled']) {
                    echo '<span title="Ne répond pas aux requêtes ping"><span class="glyphicon glyphicon-warning-sign"></span></span>';
                }
            ...
            <td style="text-align: right;">
                <button class="btn btn-xs enabled_button <?php echo !$currClass['enabled'] ? 'btn-success' : '' ?>">
                    <?php echo !$currClass['enabled'] ? 'Activer' : 'Désactiver' ?>
                </button>
...
$(function() {
        ...
        if($this.hasClass('btn-success')) {
            $.ajax("index.php?action=enable_classroom", {
                ...
                success: function(jqXHR, textStatus) {
                    ...
                    if(data.error) {
                        if(data.error == '1') alert("Impossible de changer l'état de cet auditoire.");
                        return;
                    }
    ...
    $("table.classrooms .delete_button").click(function() {
        if(!confirm('Etes vous sûr de vouloir supprimer cet élément ?')) return;
        var $this = $(this);
        ...
        $.ajax("index.php?action=remove_classroom", {
             ...
             success: function(jqXHR, textStatus) {
                ...
                if(data.error) {
                    if(data.error == '1') alert("Une erreur est survenue lors de la suppression de l'auditoire.");
                    return;
                }
