...
<div class="page_title">Créer renderer > Installation</div>
...
<div id="main_step_4">
    <form method="POST" class="form-horizontal">
        ...
        <?php if (isset($tests_success) && $tests_success) { ?>
            <div class="alert alert-success">Les tests de configuration du serveur distant (renderer) ont réussi !</div>
        <?php } ?>
        <input type="hidden" name="renderer_step" value="4"/>

        Le programme va maintenant: <br/><br/>
        <ul>
            <li>Copier les fichiers d'installation d'EZrenderer sur le serveur distant 
            [<?php echo $_SESSION['renderer_root_path'];?>]</li>
            <li>Installer EZrenderer sur le serveur distant (renderer)</li>
            <li>Mettre à jour la liste des renderers pour EZadmin et EZmanager </li>
        </ul>

        <div class="form-group">
            <input type="submit" class="btn btn-primary" name="submit_step_4_prev" value="Précédent"/>
            <input type="button" class="btn btn-primary" name="submit_step_4_next" onclick="swap('load_step_4', 'main_step_4');
        loadStepAjax(1)" value="Suivant"/>
        </div>
    </form>
</div>
...
<div id="load_step_4" style="text-align:center; display: none">
    ...
    <br/><br/>
    Copie des fichiers d'installation d'EZrenderer en cours ...
</div>
...
<div id="next_step" style="display: none">
    ...
    <form method="POST" class="form-horizontal">
        ...
        <div class="form-group">
            <input type="submit" class="btn btn-primary" name="finish_install" value="Terminer"/>
        </div>
    </form>
</div>
...
<div id="main_step_4_failed" style="display: none">
    ...
    <form method="POST" class="form-horizontal">
        <input type="hidden" name="renderer_step" value="5"/>
        <div class="form-group">
            <input type="button" class="btn btn-primary" onclick="swap('loading_img', 'main_step_4');loadStepAjax(save_step)" value="Réessayer"/>
        </div>
    </form>
</div>