...
<table class="table table-striped table-hover table-condensed table-responsive users">
    <tr>
        <th data-col="user_ID" <?php echo $input['col'] == 'user_ID' ? 'data-order="' . $input["order"] . '"' : '' ?> style="cursor:pointer;">Identifiant<?php echo ($input['col'] == 'user_ID') ? ($input['order'] == 'ASC' ? ' <span class="glyphicon glyphicon-chevron-down"></span>' : ' <span class="glyphicon glyphicon-chevron-up"></span>') : ' <span class="glyphicon glyphicon-chevron-up" style="visibility: hidden;"></span>' ?></th>
        <th>Nom</th>
        <th>Origine</th>
    ...
    <?php foreach($users as $user) {
        ...
        <tr>
            ...
            <td><span class="label <?php if($user['origin'] == 'internal') echo 'label-info'; ?>"><?php if($user['origin'] == 'internal') echo 'Interne'; else echo 'Externe'; ?></span></td>
