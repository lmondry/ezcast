        ...
        <ul>
            <li><span class="BoutonSuppAlbum">
                    <?php if ($status != 'processing' && $status != 'error') {
                        ?>
                        <a href="javascript:show_popup_from_inner_div('#popup_delete_asset_<?php echo $asset_name; ?>');">Supprimer</a>
                        <?php
                    } else {
                        ?>
                        <a href="javascript:show_popup_from_inner_div('#popup_not_available');">Supprimer</a>
                        <?php
                    }
                    ?>

                </span></li>
            <li><span class="BoutonEditer"><a href="javascript:show_edit_form('<?php echo $asset; ?>');">Editer</a></span></li>
            <li><span class="BoutonDeplacer">
                    <?php if ($status != 'processing' && $status != 'error') { ?>
                        <a href="javascript:show_popup_from_inner_div('#popup_move_asset_<?php echo $asset_name; ?>');">Déplacer</a>
                        <?php
                    } else {
                        ?>
                        <a href="javascript:show_popup_from_inner_div('#popup_not_available');">Déplacer</a>
                        <?php
                    }
                    ?>
                </span></li>
            <?php if ($public_album) { ?>
                <li><span class="BoutonDeplacerAlbumPrive">
                        ...
                        if ($status != 'processing' && $status != 'error') {
                            ?>
                            <a href="javascript:show_popup_from_inner_div('#popup_unpublish_asset_<?php echo $asset_name; ?>');">Déplacer dans l'album privé</a>
                            <?php
                        } else {
                            ?>
                            <a href="javascript:show_popup_from_inner_div('#popup_not_available');">Déplacer dans l'album privé</a>
                            <?php
                        }
                        ?>
                    </span></li>
                ...
                <?php if ($status != 'processing' && $status != 'error') { ?>
                            <a href="javascript:show_popup_from_inner_div('#popup_publish_asset_<?php echo $asset_name; ?>');">Déplacer dans l'album public</a>
                            <?php
                        } else {
                            ?>
                            <a href="javascript:show_popup_from_inner_div('#popup_not_available');">Déplacer dans l'album public</a>
                            <?php
                        }
                        ?>
                    </span></li>
                    ...
                    <?php if ($status != 'processing' && $status != 'error') { ?>
                        <a href="javascript:show_popup_from_inner_div('#popup_schedule_<?php echo $asset_name; ?>');">Programmer</a>
                        <?php
                    } else {
                        ?>
                        <a href="javascript:show_popup_from_inner_div('#popup_not_available');">Programmer</a>
                        <?php
                    }
                    ?>
                </span></li>
...
<div class="Colonne-Un-Information">
    <h1>Informations</h1>
    <p>
        <span class="infospodast">Titre&nbsp;:</span> 
    ...
    <p><span class="infospodast">Auteur&nbsp;:</span> <?php print_info($author); ?></p>
    <p><span class="infospodast">Date&nbsp;:</span> <?php print_info($date); ?></p>
    <p><span class="infospodast">Origine&nbsp;:</span> <?php if ($origin == 'SUBMIT') echo 'Soumission manuelle';
else print_info($origin); ?></p>
<?php if ($status == 'processing') {
    ?>
        <p><span class="infospodast">Statut&nbsp;:</span> <span style="color: #75940a;">en cours de traitement</span></p>
        <?php
    } else if ($status == 'failed') {
        ?>
        <p><span class="infospodast">Statut&nbsp;:</span> <span style="color: red;">Erreur d'encodage</span></p>
        <?php
    } else {
        ?>
        <p><span class="infospodast">Durée&nbsp;:</span> <?php print_info($duration); ?></p>
    ...
    if ($origin == 'SUBMIT') {
        ?>
        <p><span class="infospodast">Nom de fichier&nbsp;:</span> <?php print_info($file_name); ?></p>
    ...
    <div id="<?php echo $asset; ?>_submit" style="display: none; width: 200px; text-align: right; padding-right: 10px;">
        <input type="button" id="<?php echo $asset; ?>_cancel_button" onclick="show_edit_form('<?php echo $asset; ?>');" value="Annuler" />
        <input type="button" id="<?php echo $asset ?>_submit_button" onclick="edit_asset_data('<?php echo $album; ?>', '<?php echo $asset; ?>');" value="Sauvegarder" />
    </div>
</div>
