...
<div id="side_wrapper" <?php if($hiden_side) { echo 'style="right: -232px;"'; } ?>>
    <div id="side_menu">
        ...
        <?php if ($has_bookmark) { ?>
            <div class="bookmarks_button active"><a href="#asset_bookmarks" onclick="setActivePane('.bookmarks_button'); server_trace(new Array('3', 'bookmarks_swap', current_album, current_asset, current_tab));" title="®Display_asset_bookmarks®"></a></div>
        <?php } ?>
        ...
        <div class="settings bookmarks">
            ...
            <a class="sort-button <?php echo acl_value_get("personal_bm_order"); ?>" title="®Reverse_bookmarks_order®" href="javascript:bookmarks_sort('personal', '<?php echo (acl_value_get("personal_bm_order") == "chron") ? "reverse_chron" : "chron"; ?>', 'details');"></a>
            <ul id="bookmarks_actions">
                <li><a href="javascript:popup_bookmarks('<?php echo $_SESSION['album']; ?>', '<?php echo $_SESSION['asset']; ?>', 'custom', 'details', 'export')" title="®Export_asset_bookmarks®">®Export_bookmarks®</a></li>
                <li><a href="javascript:popup_bookmarks_import();" title="®Import_album_bookmarks®">®Import_bookmarks®</a></li>
                <li><a href="javascript:popup_bookmarks('<?php echo $_SESSION['album']; ?>', '<?php echo $_SESSION['asset']; ?>', 'custom', 'details', 'delete')" title="®Delete_asset_bookmarks®">®Delete_bookmarks®</a></li>
            </ul>
        </div>
        <div class="settings toc">
            ...
            <a class="sort-button <?php echo acl_value_get("official_bm_order"); ?>" title="®Reverse_bookmarks_order®" href="javascript:bookmarks_sort('official', '<?php echo (acl_value_get("official_bm_order") == "chron") ? "reverse_chron" : "chron"; ?>', 'details');"></a>
            <ul id="tocs_actions">
                <li><a href="javascript:popup_bookmarks('<?php echo $_SESSION['album']; ?>', '<?php echo $_SESSION['asset']; ?>', 'official', 'details', 'export')" title="®Export_asset_bookmarks®">®Export_bookmarks®</a></li>
                <?php if (acl_user_is_logged() && acl_has_album_moderation($album)) { ?>
                    <li><a href="javascript:popup_bookmarks_import();" title="®Import_album_bookmarks®">®Import_bookmarks®</a></li>
                    <li><a href="javascript:popup_bookmarks('<?php echo $_SESSION['album']; ?>', '<?php echo $_SESSION['asset']; ?>', 'official', 'details', 'delete')" title="®Delete_asset_bookmarks®">®Delete_bookmarks®</a></li>
                <?php } ?>
            </ul>
        </div>

        <!-- TODO : Settings Quiz (not really necessary) -->
    </div>
                                        ...
                                        <div class="bookmark_detail" id="bookmark_detail_<?php echo $index; ?>">
                                            ...
                                            <?php if ((acl_has_album_moderation($album) || acl_is_admin()) && !empty($_SESSION)) { ?>
                                                <div class="bookmark_options">
                                                    <a class="delete-button" title="®Delete_bookmark®" href="javascript:popup_bookmark('<?php echo $bookmark['album']; ?>', '<?php echo $bookmark['asset']; ?>', '<?php echo $bookmark['timecode']; ?>', 'custom', 'details', 'remove')"></a>
                                                    <a class="edit-button" title="®Edit_bookmark®" href="javascript:bookmark_edit('<?php echo $index; ?>', 'bookmark', '<?php echo htmlspecialchars(str_replace("'", "\'", $bookmark['title'])) ?>', '<?php echo htmlspecialchars(str_replace(array('"', "'"), array("", "\'"), $bookmark['description'])) ?>', '<?php echo htmlspecialchars(str_replace("'", "\'", $bookmark['keywords'])) ?>', '<?php echo $bookmark['level'] ?>', '<?php echo $bookmark['timecode'] ?>');"></a>
                                                    <?php if (acl_user_is_logged() && acl_has_album_moderation($album)) { ?>
                                                        <a class="copy-button" title="®Copy_bookmark®"  href="javascript:popup_bookmark('<?php echo $bookmark['album']; ?>', '<?php echo $bookmark['asset']; ?>', '<?php echo $bookmark['timecode']; ?>', 'custom', 'details', 'copy')"></a>
                                                    <?php } ?>
                                                </div>
                                            <?php } ?>
                                        </div>
            ...
            <?php } ?>
                            ...
                            <?php foreach ($official_bookmarks as $index => $bookmark) { ?>
                                ...
                                <li id="toc_<?php echo $index; ?>" class="orange level_<?php echo $bookmark['level']; ?>">
                                    ...
                                    <?php } ?>
                                        ...
                                        <?php if (acl_user_is_logged() && acl_has_album_moderation($album)) { ?>
                                            <div class="bookmark_options">
                                                <a class="delete-button" title="®Delete_bookmark®" href="javascript:popup_bookmark('<?php echo $bookmark['album']; ?>', '<?php echo $bookmark['asset']; ?>', '<?php echo $bookmark['timecode']; ?>', 'official', 'details', 'remove')"></a>
                                                <a class="edit-button orange" title="®Edit_bookmark®" href="javascript:bookmark_edit('<?php echo $index; ?>', 'toc', '<?php echo htmlspecialchars(str_replace("'", "\'", $bookmark['title'])) ?>', '<?php echo htmlspecialchars(str_replace(array('"', "'"), array("", "\'"), $bookmark['description'])) ?>', '<?php echo htmlspecialchars(str_replace("'", "\'", $bookmark['keywords'])) ?>', '<?php echo $bookmark['level'] ?>', '<?php echo $bookmark['timecode'] ?>');"></a>
                                            </div>
                                        <?php } ?>
                                        </div>
                                    </form>
                ...
                <div class="side_pane_content" id="asset_quiz">
                    ...
                    <?php if (!isset($quiz) || $quiz == false || sizeof($quiz) == 0 || $quiz === "\n") {  ?>
                        <div class="no_content">®No_quiz®</div>
                    <?php } else { ?>
                        <ul class="quiz_scroll">

                                <li class="rose level_1">
                                    <a class="item rose" style="pointer-events: none; cursor: default;">
                                        <span class="rose-title" id="quiz_title_display"><b><i>®Quiz_title® :</i></b> <?php echo $quiz[0]["title"]; ?></span>
                                    </a>
                                    ...
                                    <div class="quiz_detail" id="quiz_detail">
                                        <div class="quiz_info" id="quiz_info">
                                            ...
                                            <div class="rose-title">®Quiz_feedback® :</div>
                                        ...
                                        <div class="edit_quiz_form" id="edit_quiz">
                                            <form action="index.php" method="post" id="submit_quiz_form_edit" onsubmit="return false">
                                                ...
                                                <div class="rose-title">®Quiz_title® :</div>
                                                ...
                                                <?php foreach ($quiz as $index => $question) {  ?>
                                                    ...
                                                    <div class="rose-title">®Quiz_question® <?php echo ($index+1); ?>: </div>
                                                ...
                                                <span class="rose-title">®Quiz_feedback®: <input type="checkbox" name="feedback" id="quiz_feedback_edit" value="1" <?php if($quiz[0]['feedback'] === "1") echo "checked"; ?>></span>
                            ...
                            <?php foreach ($quiz as $index => $question) {  ?>
                                ...
                                <li id="question_<?php echo $index; ?>" class="rose level_3">
                                    ...
                                    <a class="item rose" href="javascript:player_video_seek(<?php echo $question['timecode']; ?> , '');">
                                        ...
                                        <span id="question<?php echo $index; ?>"><b><?php $indexQ = $index+1; echo "®Quiz_question® ".$indexQ; ?></b></span>
