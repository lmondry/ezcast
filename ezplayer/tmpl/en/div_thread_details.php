...
<div class="thread_header">
    <span class="visibility-logo <?php echo ($thread['studentOnly']) ? 'students' : 'all' ?> details" title="<?php echo ($thread['studentOnly']) ? 'Visible to students only' : 'Visible to everyone' ?>"></span>
    ...
    <a class="refresh-button pull-right" title="Refresh the discussion" href="javascript:thread_details_update(<?php echo $thread['id']; ?>, false)"></a>
    <a class="back-button pull-right" title="Display the list of discussions" href="javascript:threads_list_update(false);"></a>
    <br/>
    <?php
...
<span id="thread_author" class="darkgray-label">
    ...
    on <b><?php echo $creationDateVerbose; ?></b>
</span>
        ...
        <form action="index.php" method="post" id="submit_thread_form" onsubmit="return false">
            ...
            <div class="full-width">
                ...
                <input name="title" id="edit_thread_title_<?php echo $thread['id']; ?>" 
                       type="text" placeholder="Subject of the discussion" style="width: 75%" 
            ...
            <div class="message-input">
                <label>Timecode&nbsp;:
                    <span class="small">Position in seconds</span>
                </label>
...
<div id="thread-options" class="right-options">
    ...
    <?php if (($_SESSION['user_login'] == $thread['authorId']) || acl_is_admin()) { ?>
        <a class="edit-button green2 pull-right inline-block" title="Edit discussion" onclick="thread_edit_form_prepare(<?php echo $thread['id'] ?>)"></a>
        <?php if (acl_is_admin()) {
            ?>     
            <a class="delete-button green2 pull-right inline-block" title="Delete discussion" href="javascript:popup_thread('<?php echo $thread['id']; ?>', 'delete');" ></a>
<?php
        }
    }
?>
...
<?php if (count($thread['comments']) != 0) { ?>
    ...
    if ($thread['best_comment'] != NULL) {
        ...
        ?>
        <div class="cat">
            <label class="thread-cat-title"><?php echo mb_strtoupper('Best answer', 'UTF-8'); ?></label>
        </div>
    ...
    <?php } ?>
    <div class="cat">
        <label class="thread-cat-title"><?php echo mb_strtoupper('Replies', 'UTF-8'); ?></label>
    </div>
