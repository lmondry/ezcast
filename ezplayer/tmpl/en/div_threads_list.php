...
<div class="threads_header">
    ...
    <a class="refresh-button pull-right" style="margin-top: 0px; margin-right: 48px;" title="Refresh discussions" href="javascript:threads_list_update(true)"></a>
</div>
<div class="threads_list">
    ...
    if (is_array($threads) && count($threads) > 0) {
                ...
                <div class="item-thread" onclick="show_thread_details(event,<?php echo $thread['id'] ?>)">
                    <div class="item-thread-content">
                        ...
                        <?php if ($thread['nbComments'] > 0) { ?>
                            <div  class="item-thread-more item-thread-nbcomments" title="Number of comments on this discussion"><?php echo $thread['nbComments'] ?></div>
                        <?php } ?>
                        <label class="item-thread-title" ><?php echo $thread['title']; ?></label>
                        <span class="item-thread-author">
                            Edited on <b><?php echo $editDateVerbose; ?></b>
        ...
        ?>
        <span style='padding-left: 48px;'>There is no discussion yet on this video.</span>
        <?php
    }
    ?>
