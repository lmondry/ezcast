...
<div id="side_wrapper" <?php if($hiden_side) { echo 'style="right: -232px;"'; } ?>>
    <div id="side_menu">
            ...
            <div class="bookmarks_button active"><a href="#asset_bookmarks" onclick="setActivePane('.bookmarks_button');
                    server_trace(new Array('3', 'bookmarks_swap', current_album, current_asset, current_tab));" title="Display the asset bookmarks"></a></div>
                <?php
            }
            ?>
        <div class='toc_button'><a href="#album_toc" onclick="setActivePane('.toc_button');
                server_trace(new Array('3', 'bookmarks_swap', current_album, current_asset, current_tab));" title="Display official bookmarks"></a></div>
        <div class="settings bookmarks">
            <a class="menu-button" title="Bookmarks management" onclick="$(this).toggleClass('active')" href="javascript:toggle('#bookmarks_actions');"></a>
            <a class="sort-button <?php echo acl_value_get("personal_bm_order"); ?>" title="Reverse bookmarks order" href="javascript:bookmarks_sort('personal', '<?php echo (acl_value_get("personal_bm_order") == "chron") ? "reverse_chron" : "chron"; ?>', 'details');"></a>
            <ul id="bookmarks_actions">
                <li><a href="javascript:popup_bookmarks('<?php echo $_SESSION['album']; ?>', '<?php echo $_SESSION['asset']; ?>', 'custom', 'details', 'export')" title="Export the related bookmarks">Export the bookmarks</a></li> 
                <li><a href="javascript:popup_bookmarks_import();" title="Import the related bookmarks">Import the bookmarks</a></li>
                <li><a href="javascript:popup_bookmarks('<?php echo $_SESSION['album']; ?>', '<?php echo $_SESSION['asset']; ?>', 'custom', 'details', 'delete')" title="Delete bookmarks related to this record">Delete the bookmarks</a></li>                    
            </ul>
        </div>
        <div class="settings toc">
            <a class="menu-button" title="Official bookmarks management" onclick="$(this).toggleClass('active')" href="javascript:toggle('#tocs_actions');"></a>
            <a class="sort-button <?php echo acl_value_get("official_bm_order"); ?>" title="Reverse bookmarks order" href="javascript:bookmarks_sort('official', '<?php echo (acl_value_get("official_bm_order") == "chron") ? "reverse_chron" : "chron"; ?>', 'details');"></a>
            <ul id="tocs_actions">            
                <li><a href="javascript:popup_bookmarks('<?php echo $_SESSION['album']; ?>', '<?php echo $_SESSION['asset']; ?>', 'official', 'details', 'export')" title="Export the related bookmarks">Export the bookmarks</a></li>  
                <?php if (acl_user_is_logged() && acl_has_album_moderation($album)) { ?>
                    <li><a href="javascript:popup_bookmarks_import();" title="Import the related bookmarks">Import the bookmarks</a></li>
                    <li><a href="javascript:popup_bookmarks('<?php echo $_SESSION['album']; ?>', '<?php echo $_SESSION['asset']; ?>', 'official', 'details', 'delete')" title="Delete bookmarks related to this record">Delete the bookmarks</a></li>                          
<?php } ?>
            </ul>
        </div>
    </div>
        ...
        <div id="side-pane-scroll-area">
                ...
                <div class="side_pane_content" id="asset_bookmarks">
                    ...
                    if (!isset($personal_bookmarks) || $personal_bookmarks == false || sizeof($personal_bookmarks) == 0) {
                        ?>
                        <div class="no_content">There is no bookmark to display.</div>
                        <?php
                    } else {
                        ?>
                            ...
                            foreach ($personal_bookmarks as $index => $bookmark) {
                                ...
                                <li id="bookmark_<?php echo $index; ?>" class="blue level_<?php echo $bookmark['level']; ?>">
                                    <form action="index.php" method="post" id="submit_bookmark_form_<?php echo $index; ?>" onsubmit="return false">
                                        ...
                                        <span class="more"><a class="more-button small" onclick="bookmark_more_toggle('<?php echo $index; ?>', 'bookmark', $(this));"></a></span>
                                        <div class="bookmark_detail" id="bookmark_detail_<?php echo $index; ?>">
                                            ...
                                            <div class="bookmark_options">
                                                <a class="delete-button" title="Delete the bookmark" href="javascript:popup_bookmark('<?php echo $bookmark['album']; ?>', '<?php echo $bookmark['asset']; ?>', '<?php echo $bookmark['timecode']; ?>', 'custom', 'details', 'remove')"></a>
                                                <a class="edit-button" title="Edit the bookmark" href="javascript:bookmark_edit('<?php echo $index; ?>', 'bookmark', '<?php echo htmlspecialchars(str_replace("'", "\'", $bookmark['title'])) ?>', '<?php echo htmlspecialchars(str_replace(array('"', "'"), array("", "\'"), $bookmark['description'])) ?>', '<?php echo htmlspecialchars(str_replace("'", "\'", $bookmark['keywords'])) ?>', '<?php echo $bookmark['level'] ?>', '<?php echo $bookmark['timecode'] ?>');"></a>
                                                <?php if (acl_user_is_logged() && acl_has_album_moderation($album)) { ?>
                                                    <a class="copy-button" title="Add to official bookmarks"  href="javascript:popup_bookmark('<?php echo $bookmark['album']; ?>', '<?php echo $bookmark['asset']; ?>', '<?php echo $bookmark['timecode']; ?>', 'custom', 'details', 'copy')""></a>
                                                    <?php
                                                }
                                                ?>
                                            </div>
            ...
            <div class="side_pane_content" id="album_toc">
                ...
                <?php if (!isset($official_bookmarks) || $official_bookmarks == false || sizeof($official_bookmarks) == 0) {
                    ?>
                    <div class="no_content">There is no official bookmark for this course.</div>
                    <?php
                } else {
                    ?>
                    <ul class="toc_scroll">
        ...
        <?php } ?>
                                        ...
                                        <div class="bookmark_detail" id="toc_detail_<?php echo $index; ?>">
                                            ...
                                            <div class="edit_bookmark_form" id="edit_toc_<?php echo $index; ?>">            
                                                ...
                                                <br/>
                                                <div class="editButtons">
                                                    <a class="button" href="javascript: bookmark_edit_form_toggle('<?php echo $index; ?>', 'toc');">Cancel</a>
                                                    <a class="button orange" href="javascript: if(bookmark_edit_form_check('<?php echo $index; ?>', 'toc')) bookmark_edit_form_submit('<?php echo $index; ?>', 'toc');">Submit</a>
                                                </div>
                                                <br />
                                            </div>
        <?php if (acl_user_is_logged() && acl_has_album_moderation($album)) { ?>
                                                <div class="bookmark_options">
                                                    <a class="delete-button" title="Delete the bookmark" href="javascript:popup_bookmark('<?php echo $bookmark['album']; ?>', '<?php echo $bookmark['asset']; ?>', '<?php echo $bookmark['timecode']; ?>', 'official', 'details', 'remove')"></a>
                                                    <a class="edit-button orange" title="Edit the bookmark" href="javascript:bookmark_edit('<?php echo $index; ?>', 'toc', '<?php echo htmlspecialchars(str_replace("'", "\'", $bookmark['title'])) ?>', '<?php echo htmlspecialchars(str_replace(array('"', "'"), array("", "\'"), $bookmark['description'])) ?>', '<?php echo htmlspecialchars(str_replace("'", "\'", $bookmark['keywords'])) ?>', '<?php echo $bookmark['level'] ?>', '<?php echo $bookmark['timecode'] ?>');"></a>
                                                </div>
        <?php } ?>
                                        </div>
                                </form>
