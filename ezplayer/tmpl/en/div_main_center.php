...
<div class="albums">
    <div id="tuto">
        <a href="#" onclick="$('#tuto_video').toggle();" id="tuto_label">Tutorial: How to use EZplayer ?</a>
        <video id='tuto_video' width="720" controls  type="video/mp4" src="./videos/tuto.mp4" 
            ...
            Tutorial: How to use EZplayer ?</video></div>
    <?php
    if (!isset($albums) || sizeof($albums) == 0) {
        ?>
        <span>No album has been previously consulted. Please use a URL from the virtual university.</span>
        <?php
    } else {
        ?>
            ...
            foreach ($albums as $index => $album) {
                ...
                <li>
                    <a class="item <?php if ($private) echo 'private' ?>" href="javascript:show_album_assets('<?php echo $album['album']; ?>', '<?php echo $album['token']; ?>');">
                        ...
                        <?php if ($private) echo '(Private album)' ?>
                ...
                <?php if (acl_user_is_logged()) { ?>
                    <div class="album_options left">
                        <a class="up-arrow" <?php if ($index == 0) { ?>style="visibility:hidden"<?php } ?> href="javascript:album_token_move('<?php echo $album['album']; ?>', <?php echo $index; ?>, 'up');" title="Move up"></a>
                        <?php if ($index != count($albums) - 1) { ?><a class="down-arrow" href="javascript:album_token_move('<?php echo $album['album']; ?>', <?php echo $index; ?>, 'down');" title="Move down"></a><?php } ?>
                    </div>
                    <?php
                    ...
                    <div class="album_options pull-right inline-block">
                        <a  class="button-rect green pull-right inline-block share-rss" href="javascript:popup_album('<?php echo $album['album'] ?>', 'rss');">Subscribe to RSS feed</a>
                        <?php if (suffix_get($album['album']) == '-priv' || !acl_has_album_moderation($album['album'])) { ?>
                            <a class="delete-album" title="Delete the course and all related bookmarks" href="javascript:popup_album('<?php echo $album['album'] ?>', 'delete');"></a>
                        <?php } ?>
                    </div>
