...
<form id="search_form" method="post" action="index.php?action=threads_bookmarks_search" onsubmit="return false">
    ...
    <div id="advanced_search">
        <div class="search-label">Search in : </div>
        <div id="target">
            <label ><input tabindex='1' onclick="search_form_setup();" type="radio" name="target" checked="checked" value="global"><span>All</span></label>
            <label><input tabindex='2' id="album_radio" onclick="search_form_setup();" type="radio" name="target" value="album"><span>Courses</span></label>
        ...
        if (!isset($search_albums) || count($search_albums) < 1) {
            ?>
            <ul class="search_albums"><li style="border:none;">No album</li></ul>
        <?php } else { ?>
        ...
        <div class="search_current">
            <?php if (isset($_SESSION['album']) && $_SESSION['album'] != '') { ?>
                The search is only applied on course "<strong><?php echo suffix_remove($_SESSION['album']); ?></strong>"
                <?php if (isset($_SESSION['asset']) && $_SESSION['asset'] != '') { ?>
                     in record "<strong><?php echo get_asset_title($_SESSION['album'], $_SESSION['asset']); ?></strong>".
                <?php } ?>
            <?php } else { ?>
                The search is applied on all of your courses
            <?php } ?>
        </div> 
        <div class="search-label">Search among : </div>
        <div class="search_cat">
            <div class="search_fields">
                <label style="display:block;">
                    <input tabindex='5' id="cb_toc" type="checkbox" checked="checked" onclick="javascript:search_options_adjust()" 
                           name="tab[]" value="official"/>Official bookmarks</label>
                <label style="display:block;">
                    <input tabindex='6' id="cb_bookmark" onclick="javascript:search_options_adjust()" 
                           type="checkbox" checked="checked" name="tab[]" value="custom"/>Personal bookmarks</label>
                <label style="display:block;">
