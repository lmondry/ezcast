...
<span id="comment_<?php echo $comment['id']; ?>">
    <form action="index.php" method="POST">
        <?php if ($comment['approval'] == '1') { ?>
            ...
            <div class="orange-title sm"><?php echo mb_strtoupper("Answer approved by professor", "UTF-8") ?><i class="slash-sm orange"> // </i></div>
        <?php } ?>
        ...
        <div id="edit-options-<?php echo $comment['id']; ?>" style="float: right;" hidden>
            <span class="warning_edit">Warning ! When a comment is edited, the votes and approval are reset</span>
        ...
        <span class="comment-options">
            ...
            <div class="inline-block">
                ...
                <?php if (acl_has_album_moderation($thread['albumName']) || acl_is_admin()) { ?>
                    <div style="padding-top: 5px;" class="copy-button <?php echo ($comment['approval'] == '0') ? '' : 'active' ?> inline-block" title="<?php echo ($comment['approval'] == '0') ? 'Approve the answer' : 'Withdraw approval' ?>" onclick="javascript:thread_comment_approve(<?php echo $comment['id']; ?>)"></div>
                <?php } ?>
            </div>
            <!-- --- END - VOTE -------------------------------- -->
            ...
            <div class="right-options">
                ...
                <?php if ($_SESSION['user_login'] == $comment['authorId'] || acl_is_admin()) { ?>
                    <div class="inline-block">
                        <a class="edit-button green2 pull-right inline-block" title="Edit comment" onclick="javascript:thread_comment_edit_form_prepare('<?php echo $comment['id']; ?>');"></a>
                        <?php if (acl_is_admin()) { ?>
                            <a class="delete-button green2 pull-right inline-block" title="Delete comment" href="javascript:popup_thread_comment('<?php echo $comment['id']; ?>', 'delete');"></a> 
                        <?php } ?>
                    </div>
                    <?php
                }
