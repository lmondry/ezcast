...
<h3>Find here the bookmarks that can be imported.</h3><br/>
<p>Select the bookmarks you want to import in your own list. The red bookmarks will replace your own bookmarks.</p>
<br/>
<?php if (isset($imported_bookmarks) && count($imported_bookmarks) > 0) { ?>
                        ...
                        <?php if (($_SESSION['target'] == 'custom' && user_prefs_asset_bookmark_exists($_SESSION['user_login'], $bookmark['album'], $bookmark['asset'], $bookmark['timecode']))
                            ...
                            <div style="display: inline-block; width: 457px; padding-left: 8px; color:#ff0000;">
                                <a class="tooltip"><span style="padding-left: 0px;"><b><?php print_info(substr(get_user_friendly_date($bookmark['asset'], '/', false, get_lang(), false), 0, 10)); ?></b></span>
                                    ...
                                    <div class="tip"><b class="green-title">Replaces the bookmark :</b>
                                        <?php
    ...
    if ($count == 0) {
        ?>
                <br/>
                The bookmarks contained in the submitted file are not related to the selected course or record. Please choose the adequate course.
    <?php } ?>
        </ul><br/>
        <a href="#" onclick="bookmarks_import_form_submit('<?php echo $source; ?>');" id="import_button" class="simple-button blue" title="Import the selected bookmarks">Import</a>
        <a class="close-reveal-modal-button" href="javascript:close_popup();">Cancel</a>
    </form>
<?php } else { ?>
    There is no bookmark to display.
<?php } ?>
