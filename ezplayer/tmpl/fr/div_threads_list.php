...
<div class="threads_header">
    ...
    <a class="refresh-button pull-right" style="margin-top: 0px; margin-right: 48px;" title="Rafraîchir la liste" href="javascript:threads_list_update(true)"></a>
</div>
                ...
                <div class="item-thread" onclick="show_thread_details(event,<?php echo $thread['id'] ?>)">
                    <div class="item-thread-content">
                        <?php if ($thread['studentOnly'] == '1') { ?>
                            <img src="images/Generale/visibility-students.png" title="Visible uniquement par les étudiants" class="visibility"/>
                        <?php } else { ?>
                            <img src="images/Generale/visibility-all.png" title="Visible par tous" class="visibility"/>
                        <?php } ?>
                        ...
                        <span style="font-style: italic; font-size: 11px;">par <?php echo $thread['authorFullName']; ?> </span>
                        <?php if ($thread['nbComments'] > 0) { ?>
                            <div  class="item-thread-more item-thread-nbcomments" title="Nombre de commentaires sur cette discussion"><?php echo $thread['nbComments'] ?></div>
                        <?php } ?>
                        <label class="item-thread-title" ><?php echo $thread['title']; ?></label>
                        <span class="item-thread-author">
                            Modifiée le <b><?php echo $editDateVerbose; ?></b>
                            par <?php echo (isset($thread['lastEditAuthor']) && trim($thread['lastEditAuthor']) != '') ? $thread['lastEditAuthor'] : $thread['authorFullName']; ?>
                        </span>
