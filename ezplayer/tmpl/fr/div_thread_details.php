...
<div class="thread_header">
    <span class="visibility-logo <?php echo ($thread['studentOnly']) ? 'students' : 'all' ?> details" title="<?php echo ($thread['studentOnly']) ? 'Visible uniquement par les étudiants' : 'Visible par tous' ?>"></span>
    ...
    <a class="refresh-button pull-right" title="Rafraîchir la discussion" href="javascript:thread_details_update(<?php echo $thread['id']; ?>, false)"></a>
    <a class="back-button pull-right" title="Afficher la liste des discussions" href="javascript:threads_list_update(false);"></a>
    <br/>
    <?php
...
<span id="thread_author" class="darkgray-label">
    ...
    le <b><?php echo $creationDateVerbose; ?></b>
</span>
        ...
        <form action="index.php" method="post" id="submit_thread_form" onsubmit="return false">
            ...
            <div class="full-width">
                ...
                <input name="title" id="edit_thread_title_<?php echo $thread['id']; ?>" 
                       type="text" placeholder="Sujet de la discussion" style="width: 75%" 
            ...
            <div class="cancelButton" style="margin-left: 428px;">
                <a class="button" tabindex='16' href="javascript: thread_edit_form_cancel(<?php echo $thread['id']; ?>);">Annuler</a>
            </div>
            <div class="submitButton">
                <a class="button green2" tabindex='17' href="javascript: if(thread_edit_form_check(<?php echo $thread['id']; ?>)) thread_edit_form_submit(<?php echo $thread['id']; ?>,'<?php echo $thread['albumName']; ?>','<?php echo $thread['assetName']; ?>');">Sauvegarder</a>
            </div>
            <br />
        </form>
    </div>
...
<div id="thread-options" class="right-options">
    ...
    <?php if (($_SESSION['user_login'] == $thread['authorId']) || acl_is_admin()) { ?>
        <a class="edit-button green2 pull-right inline-block" title="Modifier la discussion" onclick="thread_edit_form_prepare(<?php echo $thread['id'] ?>)"></a>
        <?php if (acl_is_admin()) {
            ?>     
            <a class="delete-button green2 pull-right inline-block" title="Supprimer la discussion" href="javascript:popup_thread('<?php echo $thread['id']; ?>', 'delete');" ></a>
<?php
        }
    }
?>
...
<?php if (count($thread['comments']) != 0) { ?>
    ...
    if ($thread['best_comment'] != NULL) {
        ...
        ?>
        <div class="cat">
            <label class="thread-cat-title"><?php echo mb_strtoupper('Meilleure réponse', 'UTF-8'); ?></label>
        </div>
    ...
    <?php } ?>
    <div class="cat">
        <label class="thread-cat-title"><?php echo mb_strtoupper('Réponses', 'UTF-8'); ?></label>
    </div>
    ...
    <div id='comment_form_wrapper'>
        <form action="index.php" method="post" id="submit_comment_form" onsubmit="return false">
            ...
            <div class="cancelButton" style="margin-left: 464px;">
                <a class="button" tabindex='16' href="javascript: thread_comment_form_hide();">Annuler</a>
            </div>
            <div class="submitButton">
                <a class="button green2" tabindex='17' href="javascript: if(thread_comment_form_check()) thread_comment_form_submit();">Répondre</a>
            </div>
            <br />
        </form>
    </div>
