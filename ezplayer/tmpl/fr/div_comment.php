...
<span id="comment_<?php echo $comment['id']; ?>">
    <form action="index.php" method="POST">
        <?php if ($comment['approval'] == '1') { ?>
            ...
            <div class="orange-title sm"><?php echo mb_strtoupper("Réponse approuvée par le professeur", "UTF-8") ?><i class="slash-sm orange"> // </i></div>
        <?php } ?>
        ...
        <div id="edit-options-<?php echo $comment['id']; ?>" style="float: right;" hidden>
            <span class="warning_edit">Attention ! Lorsqu'un commentaire est modifié, les votes et approbation sont réinitialisés</span>
            <a class="button" href="javascript: thread_comment_edit_form_cancel(<?php echo $comment['id'] ?>);">Annuler</a>
            <a class="button green2" href="javascript: thread_comment_edit_form_submit(<?php echo $comment['id']; ?>);">Soumettre</a>
        </div>
        <br />
            ...
            <div id='answer_comment_form_wrapper'>
                <form action="index.php" method="post" id="submit_answer_comment_form_<?php echo $comment['id']; ?>">
                    ...
                    <div class="submitButton" style="float: right;">
                        <a class="button green2" href="javascript: if(comment_answer_form_check(<?php echo $comment['id']; ?>)) comment_answer_form_submit(<?php echo $comment['id']; ?>);">Répondre</a>
                    </div>
                    <div class="cancelButton" style="float: right;">
                        <a class="button" href="javascript: comment_answer_form_hide(<?php echo $comment['id']; ?>);">Annuler</a>
                    </div>
                    <br />
                </form>
            </div>
        ...
        <span class="comment-options">
            ...
            <div class="inline-block">
                ...
                <?php if (acl_has_album_moderation($thread['albumName']) || acl_is_admin()) { ?>
                    <div style="padding-top: 5px;" class="copy-button <?php echo ($comment['approval'] == '0') ? '' : 'active' ?> inline-block" title="<?php echo ($comment['approval'] == '0') ? 'Approuver la réponse' : 'Retirer l\'approbation' ?>" onclick="javascript:thread_comment_approve(<?php echo $comment['id']; ?>)"></div>
                <?php } ?>
            </div>
            <!-- --- END - VOTE -------------------------------- -->

            <div class="right-options">
                <a class="button-empty green2 pull-right inline-block" onclick="javascript:comment_answer_form_show('<?php echo $comment['id']; ?>');">Répondre</a>
                <?php if ($_SESSION['user_login'] == $comment['authorId'] || acl_is_admin()) { ?>
                    <div class="inline-block">
                        <a class="edit-button green2 pull-right inline-block" title="Modifier le commentaire" onclick="javascript:thread_comment_edit_form_prepare('<?php echo $comment['id']; ?>');"></a>
                        <?php if (acl_is_admin()) { ?>
                            <a class="delete-button green2 pull-right inline-block" title="Supprimer le commentaire" href="javascript:popup_thread_comment('<?php echo $comment['id']; ?>', 'delete');"></a> 
                        <?php } ?>
                    </div>
                    <?php
                }
