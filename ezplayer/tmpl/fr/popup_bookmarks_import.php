...
<h3>Voici la liste des signets disponibles pour l'importation.</h3><br/>
<p>Sélectionnez les signets que vous souhaitez importer dans votre liste. Les signets apparaissant en rouge vont remplacer vos signets personnels.</p>
<br/>
<?php if (isset($imported_bookmarks) && count($imported_bookmarks) > 0) { ?>
    <form action="index.php?action=bookmarks_import" method="post" id="select_import_bookmark_form" name="import_bookmark_form" onsubmit="return false">
        ...
        <ul>
            <li style="border-bottom: solid 1px #cccccc;"><input type="checkbox" onclick="toggle_checkboxes(this, 'import_selection[]')" name="check_all"/><span class="blue-title"><b>Date</b></span><span class="blue-title"><b>Signet</b></span></li>
                        ...
                        <?php if (($_SESSION['target'] == 'custom' && user_prefs_asset_bookmark_exists($_SESSION['user_login'], $bookmark['album'], $bookmark['asset'], $bookmark['timecode']))
                            ...
                            <div style="display: inline-block; width: 457px; padding-left: 8px; color:#ff0000;">
                                <a class="tooltip"><span style="padding-left: 0px;"><b><?php print_info(substr(get_user_friendly_date($bookmark['asset'], '/', false, get_lang(), false), 0, 10)); ?></b></span>
                                    ...
                                    <div class="tip"><b class="green-title">Remplace le signet :</b>
                                        <?php
                                        ...
                                        <div style="border-top:1px dotted #cccccc; width: 400px; padding-top: 8px">
                                            ...
                                            <b>Mots clés: </b><?php print_info($personal_bookmark['keywords']); ?>
                                        </div>
                                    </div></a>
                            </div>
                            <?php } else { ?>
    ...
    if ($count == 0) {
        ?>
                <br/>
                Les signets du fichier que vous souhaitez importer ne correspondent pas au cours ou l'enregistrement sélectionné. Veuillez choisir le cours approprié.
    <?php } ?>
        </ul><br/>
        <a href="#" onclick="bookmarks_import_form_submit('<?php echo $source; ?>');" id="import_button" class="simple-button blue" title="Importer les signets sélectionnés">Importer</a>
